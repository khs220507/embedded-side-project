...
NUCLEO-F401RE의 USART2 송수신을 DMA 방식으로 처리하는 학습 프로젝트다. `02_UART`의 LED 문자열 명령 기능을 유지하면서 CPU 대신 DMA1이 메모리와 USART2 사이의 데이터를 옮긴다.
...
Debug 빌드에서 `build/Debug/03_UART_DMA.elf` 생성을 확인했다. STM32CubeProgrammer로 Flash 기록과 Verify를 완료했고, 실제 보드에서 DMA 송수신과 LED 명령 동작을 확인했다.
